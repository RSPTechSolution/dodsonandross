  ...
  public function build(string $id, array $configuration = [], bool $wrapper = TRUE): array {
    ...
    if ($access->isAllowed()) {
      ...
      if ($wrapper && is_array($build['content']) && !Element::isEmpty($build['content'])) {
        $attributes = [];
        // Add some specific classes for ajax block type.
        if (isset($block_plugin->getConfiguration()['block_ajax'])) {
          if ($block_plugin->getConfiguration()['block_ajax']['is_ajax']) {
            $attributes = [
              'class' => ['block-ajax-block'],
            ];
          }
        }
        $build += [
