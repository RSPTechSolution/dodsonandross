...
class ModulesListFormTest extends BrowserTestBase {
  ...
  public function testModulesListForm() {
    ...
    // Changing the values for first time.
    ...
    $this->assertSession()->pageTextContains('User have now as weight: -2');
    ...
    $this->assertEquals(15, $this->getSession()->getPage()->findField('modules[page_cache][weight]')->getValue());
    ...
    $this->assertEquals(-2, $this->getSession()->getPage()->findField('modules[user][weight]')->getValue());
    ...
    $this->assertEquals(15, $modules_weight['page_cache']);
    $this->assertEquals(-3, $modules_weight['modules_weight']);
    $this->assertEquals(0, $modules_weight['system']);
    $this->assertEquals(-2, $modules_weight['user']);

    // Changing the values for second time.
    // The new modules weight values to send.
    $edit = [
    ...
    $this->assertSession()->pageTextContains('User have now as weight: 0');

    // Checking the value in the selects.
    $this->assertEquals(15, $this->getSession()->getPage()->findField('modules[system][weight]')->getValue());
    $this->assertEquals(-3, $this->getSession()->getPage()->findField('modules[page_cache][weight]')->getValue());
    $this->assertEquals(0, $this->getSession()->getPage()->findField('modules[user][weight]')->getValue());

    // Getting the modules weight values.
    $modules_weight = $this->container->get('config.factory')->get('core.extension')->get('module');
    // Verifying the weight values.
    $this->assertEquals(15, $modules_weight['system']);
    $this->assertEquals(-3, $modules_weight['page_cache']);
